...
namespace StudentsListApp.Views;
...
public partial class StudentsView : ContentPage
{
    ...
    public StudentsView(ClassModel selectedClass)
    {
        ...
        BindingContext = new StudentManagaer();
        LoadStudents();
    }
    ...
    private void SaveStudents()
    {
        try
        {
            if (BindingContext is StudentManagaer student)
            {
    ...
    private void onGetRandomStudentClicked(object sender, EventArgs e)
    {
        try
        {
            if (BindingContext is StudentManagaer student && student.Students.Count > 0)
            {
                Random random = new Random();
                int id = random.Next(student.Students.Count);
                var randomStudent = student.Students[id];

                DisplayAlert("Random Student", $"Name: {randomStudent.Name} Last Name: {randomStudent.LastName} Number: {randomStudent.Number}", "OK");
            }
            else
            {
